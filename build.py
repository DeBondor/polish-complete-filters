...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Aggregate upstream filter lists into a single uBlock Origin–compatible list.",
    )
    ...
    p.add_argument("-o", "--output", type=Path, default=DEFAULT_OUTPUT,
                   help="Path to the output filter list (default: polish-complete-filters.txt)")
    ...
    p.add_argument("--dry-run", action="store_true",
                   help="Fetch and parse but do not write output; print stats only")
    return p.parse_args()
...
def main() -> None:
    ...
    log.info("Fetching with %d workers ...", args.jobs)
    t0 = time.monotonic()
    results = fetch_all(urls, args.jobs)
    ...
    if args.dry_run:
        ...
        log.info("Wrote %s (%.1f MB, %d lines)",
                 args.output, size_mb, len(output_lines))
